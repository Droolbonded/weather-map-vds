...
import {
  ...
  MapPin, Edit2, Check, X, AlertTriangle, Flame, ShieldOff,
} from "lucide-react";
...
export default function DevicesPage() {
  ...
  const handleSimulate = (device: Device) => {
    simulateReading.mutate({ id: device.id }, {
      onSuccess: () =>
        toast({ title: "Veri uretildi", description: `${device.name} icin olcum kaydedildi.` }),
      onError: () =>
  ...
  const handleStartFire = (device: Device) => {
    startFire.mutate({ id: device.id }, {
      onSuccess: () => {
        toast({ title: "Yangin basladi", description: device.name, variant: "destructive" });
        invalidateDevices();
      },
      onError: () =>
        toast({ title: "Hata", description: "Yangin baslatılamadı", variant: "destructive" }),
    });
  };

  const handleStopFire = (device: Device) => {
    stopFire.mutate({ id: device.id }, {
      onSuccess: () => {
        toast({ title: "Yangin sonduruldu", description: device.name });
        invalidateDevices();
      },
      onError: () =>
        toast({ title: "Hata", description: "Yangin sondurulemedi", variant: "destructive" }),
    });
  };
            ...
            <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-3">
              ...
              {onFire > 0 && (
                <span className="flex items-center gap-1 text-red-400 font-medium">
                  <Flame className="w-3 h-3" /> {onFire} yangin aktif
                </span>
              )}
            </p>
          </div>
              ...
              {(devices as Device[]).filter(d => d && d.id).map((device, i) => (
                ...
                >
                  {confirmDeleteId === device.id ? (
                    ...
                    <div className={`border rounded-lg px-4 py-3 flex items-center gap-4 group transition-colors ${
                      device.fireMode
                        ? "bg-red-950/30 border-red-900/50 hover:border-red-700/50"
                      ...
                      <div className="flex-1 min-w-0">
                        {editingId === device.id ? (
                          ...
                          <div className="flex items-center gap-2">
                            <span
                              className={`text-sm font-medium truncate ${device.fireMode ? "text-red-300" : ""}`}
                              data-testid={`text-device-name-${device.id}`}
                            >
                              {device.name}
                            </span>
                            {device.fireMode && (
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        ...
                        {device.isVirtual && !device.fireMode && (
                          <Button
                        ...
                        {/* Fire button — only for virtual devices */}
                        {device.isVirtual && (
                          device.fireMode ? (
                            <Button
                            ...
                            <Button
                              ...
                              size="sm" variant="ghost"
                              className="h-7 px-2 text-xs gap-1 text-orange-400 hover:text-red-400 hover:bg-red-500/10"
